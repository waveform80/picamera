...
* A port (:class:`MMALControlPort` and derivatives) is either a control port,
  ...
  - Control ports are used to accept and receive commands and errors messages
    All MMAL components have a control port, but in picamera they're almost
    never used except for global error handling.
  ...
  - Ports have a format (:attr:`~MMALPort.format`) which dictates the format of
    image/frame accepted or generated by the port (YUV, RGB, JPEG, H.264, etc.)

  - Video ports (the sort of port you're most likely to deal with in picamera),
    have a frame-rate (:attr:`~MMALVideoPort.framerate`) which specifies the
...
* Data is written to / read from ports in buffers (:class:`MMALBuffer`).
  ...
  - The size of a buffer is dictated by the format and frame-size of the port
    ...
    (:attr:`MMALBuffer.size`) cannot be altered, but the buffer can contain any
    amount of data up its allocation size (the actual length of data in a
    buffer is stored in :attr:`MMALBuffer.length`).

  - The number of buffers belonging to a port is fixed and cannot be altered
    without disabling the port, reconfiguring it and re-enabling it. The more
    buffers a port has, the less likely it is that the pipeline will have to
    drop frames because a component has overrun, but the more GPU memory is
    required.
  ...
  - When a connection exists between two ports, the connection continually
    ...
    then returns the buffers to their respective ports (this is a bit of a lie;
    various tricks are pulled under the covers to minimize the amount of data
    copying that *actually* occurs, but as a mental model of what's going on
    it's reasonable).
...
* The buffer sizes look unrealistic. For example, 7680 bytes is nowhere near
  enough to hold 320 * 240 * 1.5 bytes (YUV-420 requires 1.5 bytes per pixel).
...
hard to auto-configure things so that they work. Usually, auto-configuration is
based upon the *output* port being connected so it's important to get that
configuration right, but you don't generally need to worry about the input
port.
...
.. code-block:: pycon
    ...
    >>> camera.control.params[mmal.MMAL_PARAMETER_BRIGHTNESS] = '3/6'
...
sounds. An output port that is participating in a connection needs nothing
more: it knows where it's data is going. However, an output port *without* a
connection requires a callback function to be assigned so that something can be
